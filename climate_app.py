...
def precipitation():
	...
	results = session.query(Measurements.date,Measurements.prcp).filter(Measurements.date >= "04-02-2017").all()
...
def temperature():
	...
	year_tobs = []
	results = session.query(Measurements.tobs).filter(Measurements.date >= "04-02-2017").all()
